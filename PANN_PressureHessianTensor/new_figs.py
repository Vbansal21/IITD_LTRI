...
def construct_anisotropic_Q(H_np):
    """H_np: (N,3,3) full pressure Hessian"""
    trace = H_np[:, 0, 0] + H_np[:, 1, 1] + H_np[:, 2, 2]
    I = np.eye(3)[None, :, :]
    Q_np = H_np - (trace[:, None, None] / 3.0) * I
    return Q_np
...
def main():
    ...
    # Quick diagnostic on raw pressure Hessian vs expected isotropic part
    trace_H = H_np[:, 0, 0] + H_np[:, 1, 1] + H_np[:, 2, 2]
    Z_expected = np.sum(A_np * A_np, axis=(1, 2))
    print(f"Tr[H] mean: {trace_H.mean():.3e}")
    print(f"Z = A:A mean: {Z_expected.mean():.3e}")
    print(f"Correlation: {np.corrcoef(trace_H, Z_expected)[0,1]:.4f}")

    # Construct anisotropic pressure Hessian by removing isotropic component
