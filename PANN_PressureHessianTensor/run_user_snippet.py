#!/usr/bin/env python3
"""
Minimal wrapper around the user's provided script.
Only additions: save plots to files (no logic changes).
"""
import os
...
def plot_figure_9(alignments, save_path=None):
...
def plot_figure_10(alignments, save_path=None):
    """Plot Figure 10: Vorticity alignments with pressure Hessian eigenvectors"""

    labels = [
        ('vorticity_eap', 'ω · ea^p'),
        ('vorticity_ebp', 'ω · eb^p'),
        ('vorticity_ecp', 'ω · ec^p')
    ]
...
def plot_figure_11(w_values, save_path=None):
    ...
    bins = np.linspace(0, np.max(w_clean) if w_clean.size else 1.0, 50)
...
def plot_figure_12(bins, phi_values, save_path=None):
...
def main_analysis(vel_grad_path='velGrad.mat', pressure_hessian_path='PH.mat', out_dir='figs'):
    ...
    plot_figure_9(alignments_fig9, save_path=out / 'fig9_align_s_vs_Qhat0.png')

    print("Generating Figure 10...")
    plot_figure_10(alignments_fig10, save_path=out / 'fig10_align_vorticity_vs_Qhat0.png')

    print("Generating Figure 11...")
    plot_figure_11(w_values, save_path=out / 'fig11_pdf_omega.png')

    print("Generating Figure 12...")
    plot_figure_12(bins, phi_values, save_path=out / 'fig12_phi_vs_qe2.png')
...
if __name__ == "__main__":
    ...
    vel = 'PANN_PressureHessianTensor/velGrad.mat'
    ph  = 'PANN_PressureHessianTensor/PH.mat'
    out = 'PANN_PressureHessianTensor/figs'
    main_analysis(vel, ph, out)
